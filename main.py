...
def makePlaylistRequest(ID):
    while True:
        global session
        try:
            session.get("https://distrokid.com/wheel")
            session.get("https://distrokid.com/wheel/connect/?scope=playlist-modify-public+user-follow-modify", allow_redirects=True)
            place = session.post("https://distrokid.com/api/wheel/", data={'songId': ID})

            try:
                pos = place.json()["positions"]
                pos.sort()
                if webhook == True:
                    post(webhookURL, data={"content": "@here Your song has been pushed to position " + str(pos[0]) + "!"})
                else:
    ...
    while True:
        ...
        if inp == "y":
            ...
            while True:
                ...
                M.select("INBOX")
                idler = Idler(M, makePlaylistRequest, [songId])
                idler.start()
                print("Successfully Started")
                idler.join()
                M.logout()
        if inp == "n":
            makePlaylistRequest(songId)
            break
